...
% ћатематическа€ модель вентильного двигаел€
F=[-x1+x2*x3+u1;
    -x2-x1*x3-0.568*x3+0.1976;
    5.4574*(x2-x3)-0.5434];
% Ќахождение равновесных точек 
S = solve('-x1+x2*x3+u1=0','-x2-x1*x3-0.568*x3+0.1976=0','5.4574*(x2-x3)-0.5434=0','x1,x2,x3');
Sx1=vpa(S.x1,4)
Sx2=vpa(S.x2,4)
Sx3=vpa(S.x3,4)
% Ћинеаризаци€ в окрестности точки равновеси€
for i=1:numel(S.x1)
    dF(i,1) = diff(F(i),x1);
    dF(i,2) = diff(F(i),x2);
    dF(i,3) = diff(F(i),x3);
end
% ’арактеристические матрицы
...
% ’арактеристические полиномы матриц
...
xlabel({'корневой годограф дл€ 1-го','характеристического полинома'})
axes(handles.axes2)
plot(real(L2),imag(L2),'b*'),grid on
xlabel({'корневой годограф дл€ 2-го','характеристического полинома'})
axes(handles.axes3)
plot(real(L3),imag(L3),'b*'),grid on
xlabel({'корневой годограф дл€ 3-го','характеристического полинома'})
