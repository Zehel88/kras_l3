...
% T=20;
% 
% x1=X(fix(numel(X(:,1))/2),1);
% x2=X(fix(numel(X(:,1))/2),2);
% x3=X(fix(numel(X(:,1))/2),3);
% for j=1:length(mu)    
% 
%     for k=1:T
%         x_1(1)=x1*(1+(10^-7)*rand); 
...
T=50;
% Интервал движения по соседней траектории
t=0.1;
% Шаг интегрирования
h=0.001;
% Значения бифуркационного параметра
mu=[109;157;-20;-5;0;5;20;50];
% Кол-во точек опорной траетории
N=numel(0:t:T);
% Расчет опорной траектории
% Начальная точка на аттракторе
% ind=100;
% X0(:,1)=[X(ind,1);X(ind,2);X(ind,3)];
X0(:,1)=[-24.78;-1.416;-2.158];
for M=1:numel(mu)
% Начальная точка соседней траектории
b=1.01;
Xn(:,1)=X0(:,1)*b;
% Определение величины начального отклонения
p0=norm(Xn(:,1)-X0(:,1));
p_buf=0;
i=1;
for k=1:N
%   Интегрирование уравнения движения вдоль опорной траектории
    X0(1,k+1)=X0(1,k)+h*(-10*X0(1,k)+(X0(2,k)-X0(3,k))*mu(M));
    X0(2,k+1)=X0(2,k)+h*(12-X0(2,k)+X0(1,k)*X0(3,k));
    X0(3,k+1)=X0(3,k)+h*(-X0(1,k)*X0(2,k)-X0(3,k)+12);  
%   Переопределение соседней траектории и начального отклонения 
%   В случае большого отклонения
    if p_buf>0.01
       Xn(:,k)=X0(:,k)*b;
       i=i+1;
       p0(i)=norm(Xn(:,k)-X0(:,k));
       lam(i)=log(p(i-1)/p0(i-1));
    end
%   Интегрирование уравнения движения вдоль соседней траектории
    Xn(1,k+1)=Xn(1,k)+h*(-10*Xn(1,k)+(Xn(2,k)-Xn(3,k))*mu(M));
    Xn(2,k+1)=Xn(2,k)+h*(12-Xn(2,k)+Xn(1,k)*Xn(3,k));
    Xn(3,k+1)=Xn(3,k)+h*(-Xn(1,k)*Xn(2,k)-Xn(3,k)+12);
%   Определение отклонения в конце текущего отрезка
    p(i)=norm(Xn(:,k+1)-X0(:,k+1));
    p_buf=p(i);
