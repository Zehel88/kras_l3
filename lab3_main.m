...
% Функция для задания 1.8
function dxdt = odefun18(t, x, mu)
dxdt = zeros(3, 1);
dxdt(1) =x(2)*(x(3)-1+x(1)^2)+0.87*x(1);
dxdt(2) =x(1)*(3*x(3)+1-x(1)^2)+0.87*x(2);
dxdt(3) = -2*x(3)*(mu+x(1)*x(2));


% Функции обработчики кнопок выбора задания
...
% Выполнение скрипта соответствующий выбранному заданию            
    switch active_var
        case 1
%% ========================        1.2  ===================================
clc
% Интерфесные преобразования
...
% Обьявление символьных переменынх 
syms x1 x2 x3 u1
% % ========================================Аттрактор вентильного двигателя
F=[-x1+x2*x3+u1;
    -x2-x1*x3+0.568*x3+0.1976;
    5.4574*(x2-x3)+0.5434];
% Встроенные функции соответствующие уравнениям системы
F1_in=inline(F(1))
F2_in=inline(F(2))
F3_in=inline(F(3))
...
F=[-10*x1+m*(x2-x3);
    12-x2+x1*x3;
    -x1*x2-x3+12];
% Встроенные функции соответствующие системе уравннеий
F1_in=inline(F(1))
F2_in=inline(F(2))
F3_in=inline(F(3))
% Бифуркационный параметр
...
syms x1 x2 x3 m
% Аттрактор Холмса
F=[x2*(x3-1+x1^2)+0.87*x1;
   x1*(3*x3+1-x1^2)+0.87*x2;
   -2*x3*(m+x1*x2)];
F1_in=inline(F(1))
F2_in=inline(F(2))
F3_in=inline(F(3))
...
x_3(1)=0.6;
for i=1:(length(t)-1)
    x_1(i+1)=x_1(i)+h*(x_2(i)*(x_3(i) - 1 + x_1(i)^2)+0.87*x_1(i));
    x_2(i+1)=x_2(i)+h*(x_1(i)*(3*x_3(i)+1-x_1(i)^2)+0.87*x_2(i));
    x_3(i+1)=x_3(i)+h*(-2*x_3(i)*(mu_kr+x_1(i)*x_2(i)));
end
axes(handles.axes4)
...
h = 0.02;
 t=0:h:50;
x_1=zeros((length(t)-1),Tt);
x_2=zeros((length(t)-1),Tt);
x_3=zeros((length(t)-1),Tt);
x1(1,1)=1.4;
x2(1,1)=-0.8;
x3(1,1)=0.6;
for j=1:length(mu)    
    for k=1:Tt
        x_1(1,k)=x1(1,1)*(1+(10^-7)*rand); 
        x_2(1,k)=x2(1,1)*(1+(10^-7)*rand);
        x_3(1,k)=x3(1,1)*(1+(10^-7)*rand);

        for i=1:(length(t)-1)      
            x_1(i+1,k)=x_1(i,k)+h*(x_2(i,k)*(x_3(i,k)-1+x_1(i,k)^2)+0.87*x_1(i,k));
            x_2(i+1,k)=x_2(i,k)+h*(x_1(i,k)*(3*x_3(i,k)+1-x_1(i,k)^2)+0.87*x_2(i,k));
            x_3(i+1,k)=x_3(i,k)+h*(-2*x_3(i,k)*(mu(j)+x_1(i,k)*x_2(i,k)));
        end
...
% % =============================Определение показателя Ляпунова (1 подход) 
mu_ex=[1.7];
set(handles.uitable1,'ColumnName',{'1.7'})
    % Шаг интегрирования
  h=0.01;
% Интервал моделирования
  T=0:h:50;
...
for ii=1:numel(mu_ex)  
% Опорная траектория
[Ti, Xi] = ode45(@odefun18,T,[1.4;-0.8;0.6], [], mu_ex(ii));
% Точки на опорной траетории       
T_=Ti(1:inv(h):end);
X_=Xi(1:inv(h):end,:); 
...
%         r0 = delta * sqrt(3);
r0=norm(X_(k, :)*bet-X_(k,:));
        xxx = ode45(@odefun18, [T_(k) : 0.01 : T_(k + 1)],X_(k, :)*bet,[], mu_ex(ii));
        r1 = norm(xxx.y(:,end) - X_(k + 1, :)');
        lam(k)=log(r1/r0);
    end
 L1(ii)=1/T(end)*sum(lam)
...
for i=1:N_int
    for j=1:N_var-1
       ...
       x_2=X(2,j);
       x_3=X(3,j);
       X(1,j+1)=x_1+h*(x_2*(x_3-1+x_1^2)+0.87*x_1);
       X(2,j+1)=x_2+h*(x_1*(3*x_3+1-x_1^2)+0.87*x_2);
       X(3,j+1)=x_3+h*(-2*x_3*(mu_ex(M)+x_1*x_2));
 
        % линеаризованная система
    A=[2*x_1*x_2+0.87 x_1^2+x_3-1 x_2;
        -3*x_1^2+3*x_3+1 0.87 3*x_1;
        -2*x_2*x_3 -2*x_1*x_3 -2*mu_ex(M)-2*x_1*x_2];
        % уравнение в вариациях
        V(:,j+1)=V(:,j)+h*A*V(:,j);
    end
    % нормированный вектор принимаем в качестве начального условия
